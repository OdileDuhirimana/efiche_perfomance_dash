...
from app.services.sprint_utils import (
    was_sprint_active_in_week_primary_only,
    was_sprint_active_in_period,
    get_active_sprint_ids_for_period
)
...
def _normalize_date_to_utc(dt):
    ...
    Use: Internal helper to ensure consistent UTC timezone for date comparisons across chart calculations.
...
def calculate_weekly_planned_vs_done(df_issues, start_date, num_weeks=12, df_sprints=None):
    """
    Calculate weekly planned vs done data.

    IMPORTANT: This function expects df_issues to be pre-filtered by apply_standard_filters() 
    to ensure data consistency across all charts. The df_issues should already have assignee, 
    issue_type, and date window filters applied before calling this function.

    Logic: Generates week ranges from start_date. For each week, applies global sprint filter (issues in active sprints during week).
    Calculates planned (Created OR Updated during week) and done (Resolved during week AND Status == 'Done').
    Calculates completion rate (done / planned * 100). Returns DataFrame with weekly metrics.
    
    Both planned and done calculations use the same filtered week_issues dataset to ensure consistency.

    Use: Used by weekly planned vs done chart to show planned activities and completed work per week.

    Args:
        df_issues: DataFrame with issues (should be pre-filtered by apply_standard_filters)
        start_date: Start date for analysis
        num_weeks: Number of weeks to analyze (default 12)
        df_sprints: Optional sprint DataFrame

    Returns:
        DataFrame with columns: Week, Week Label, Week Number, Start Date, End Date, Planned, Done, Completion Rate
    """
    ...
    for week_info in weeks_data:
        ...
        week_issues_mask = df_issues.apply(
            lambda row: was_sprint_active_in_week_primary_only(row, week_start, week_end, df_sprints),
            axis=1
        )
        week_issues = df_issues[week_issues_mask].copy()

        planned_activities = filter_planned_activities(week_issues, week_start, week_end)
...
def calculate_weekly_flow(df_issues, start_date, num_weeks=12, df_sprints=None):
    """
    ...
    The 'done' count uses the same count_done_during_period() function as calculate_weekly_planned_vs_done()
    with identical parameters, ensuring consistency between weekly planned vs done and weekly flow charts.
    
    Logic: Generates week ranges from start_date. For each week, applies global sprint filter. Calculates done (Resolved during week AND Status == 'Done'),
    carry-over (Created before week AND (Updated OR Resolved during week)), and in-progress (using changelog to check status at week_end for issues not resolved during week).
    Uses changelog transitions for accurate in-progress detection. Falls back to status column if changelog unavailable.
    
    Use: Used by weekly flow chart to show work state distribution (done, in progress, carry-over) per week.
    ...
    Returns:
        DataFrame with columns: Week, Week Label, Week Number, Start Date, End Date, Done, In Progress, Carry Over, New Issues, Total Active
    """
    ...
    for week_idx, week_info in enumerate(weeks_data):
        ...
        carry_over = filter_carry_over_activities(week_issues, week_start, week_end)
        ...
        if 'Status Transitions' in week_issues.columns:
            ...
            for _, row in week_issues.iterrows():
                ...
                if resolved_dt is not None and not pd.isna(resolved_dt):
                    if week_start <= resolved_dt <= week_end:
                        continue
                ...
                if transitions:
                    try:
                        ...
                        status_at_end = get_status_at_date(transitions, week_end)
...
def calculate_weekly_lead_time(df_issues, start_date, num_weeks=12, df_sprints=None):
    """
...
def calculate_task_load_per_assignee(df_issues, period_start, period_end, df_sprints=None):
    ...
    SIMPLIFIED LOGIC:
    - Filter: Issues in active sprints during period
    ...
    if df_sprints is not None and not df_sprints.empty:
        active_sprint_ids = get_active_sprint_ids_for_period(period_start_utc, period_end_utc, df_sprints)

        if 'Primary Sprint Id' in df_issues.columns:
            period_issues = df_issues[df_issues['Primary Sprint Id'].isin(active_sprint_ids)].copy()
        else:
            period_mask = df_issues.apply(
                lambda row: was_sprint_active_in_period(row, period_start_utc, period_end_utc, df_sprints),
                axis=1
            )
            period_issues = df_issues[period_mask].copy()
    else:
        period_mask = df_issues.apply(
            lambda row: was_sprint_active_in_period(row, period_start_utc, period_end_utc, df_sprints),
            axis=1
...
def calculate_company_trend(df_issues, period_start, num_months=6, period_end=None, df_sprints=None):
    ...
    max_months_to_check = num_months * 3
    while months_with_data < num_months and months_checked < max_months_to_check:
        ...
        if period_start_utc:
            period_start_month = period_start_utc.replace(day=1)
            if month_start < period_start_month:
                break
        ...
        if df_sprints is not None and not df_sprints.empty:
            active_sprint_ids = get_active_sprint_ids_for_period(month_start_utc, month_end_utc, df_sprints)

            if 'Primary Sprint Id' in df_issues.columns:
                month_issues = df_issues[df_issues['Primary Sprint Id'].isin(active_sprint_ids)].copy()
            else:

                month_mask = df_issues.apply(
                    lambda row: was_sprint_active_in_period(row, month_start_utc, month_end_utc, df_sprints),
                    axis=1
                )
                month_issues = df_issues[month_mask].copy()
        else:
        ...
        if current_date.month == 1:
            current_date = datetime(current_date.year - 1, 12, 1, tzinfo=current_date.tzinfo)
        else:
...
def calculate_qa_vs_failed(df_issues, period_start, period_end, group_by='sprint', df_sprints=None):
    ...
    if group_by == 'sprint':
        ...
        while current_date <= period_end_utc:
            ...
            week_issues_mask = period_issues.apply(
                lambda row: was_sprint_active_in_week_primary_only(row, current_date, week_end, df_sprints),
                axis=1
            )
            week_issues = period_issues[week_issues_mask].copy()
...
def calculate_rework_ratio(df_issues, start_date, num_weeks=12, df_sprints=None):
    """
